...
function random_cc_clifford(n)
    ...
    p = randperm(n)
    mX = view(xz, 1:2:2n, 1:2n)
    mX[p, :] = mX
    mZ = view(xz, 2:2:2n, 1:2n)
    mZ[p, :] = mZ
    for k in 1:n
        s[k] = m4(s[k] + 2*rand(Bool))
    end
    return Clifford(xz, s)
end
...
function entropy(state::AbstractArray{Bool, 2}, rg)
    m, n = size(state)
    n = div(n, 2)
    rg_bar = setdiff(1:n, rg)
    indices = spin_to_binary_indices(rg_bar)
    rk = binary_rank(state[:, indices])
    return length(rg) - m + rk
end

function entropy(state::StabState, rg)
    m, n = size(state)
    return entropy(view(state.xz, 1:m, :), rg)
end


function left_ee_on_all_cuts(state::AbstractArray{Bool, 2})
    m, n = size(state)
    n = div(n, 2)
    mat = state[:, 2n:-1:1]
...
function left_ee_on_all_cuts(state::StabState)
    m, n = size(state)
    return left_ee_on_all_cuts(view(state.xz, 1:m, :))
end
