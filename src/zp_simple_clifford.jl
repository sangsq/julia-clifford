...
copy(state::StabState) = StabState{T}(copy(state.xz))
...
function check_xz(xz::Matrix{T}) where T
    n = div(size(xz,1),2)
    for i in 1:n
        for j in 1:n
            @assert zp_symplectic_inner(xz[i, :], xz[j, :])==zero(T)
            @assert zp_symplectic_inner(xz[i+n, :], xz[j+n, :])==zero(T)
            @assert zp_symplectic_inner(xz[i, :], xz[j+n, :]) == (i==j ? one(T) : zero(T))
        end
    end
end


function tproduct(states::AbstractVector{StabState{T}}) where T
...
function cconj(state::StabState{T}) where T
    tmp = copy(state.xz)
    tmp[:, 2:2:end] .*= T(-1)
    return StabState{T}(tmp)
end
...
function epr_pairs(T, n)
    xz = zeros(T, 4n, 4n)
    for i in 1:n
        xz[2i-1, 2i-1] = xz[2i-1, 2n+2i-1] = T(1)
        xz[2i, 2i] = xz[2i, 2n+2i] = T(1)
        xz[2n+2i-1, 2i] = T(1)
        xz[2n+2i, 2n+2i-1] = -T(1)
    end
    # check_xz(xz)
    return StabState{T}(xz)
end
...
function random_state(T, n)
    tmp = zp_random_symplectic_matrix(T, n)
    xz = zeros(T, 2n, 2n)
...
function random_clifford(T, n)
    xz = zp_random_symplectic_matrix(T, n)
    return Clifford{T}(xz)
end

function random_css_clifford(T, n)
    xz = zp_random_sign_free_symplectic_matrix(T, n)
    return Clifford{T}(xz)
end
...
function clifford_action!(state::StabState{T}, clifford::Clifford{T}, positions) where T
    n_act = length(positions)
    @assert size(clifford.xz, 1) == n_act * 2
    xz = state.xz
    n = nsites(state)
    indices = spin_to_binary_indices(positions)
    tmp_xz = zeros(T, 2 * n_act)
    for k in 1:2n
        tmp_xz .= zero(T)
        for j in 1:2n_act
            for l in 1:2n_act
                tmp_xz[l] += clifford.xz[j, l] * xz[k, indices[j]]
            end
        end
        xz[k, indices] = tmp_xz
    end
...
"""
function measure_out!(state::StabState{T}, sites) where T
    xz = state.xz
    ...
    for i in sites
        bad_rows = Int[]
        for k in 1:2n
            if remain_rows[k] && !iszero(xz[k, 2i])
                push!(bad_rows, k)
            end
        end
        ...
        tmp = xz[the_row, remain_cols]
        col_inds = findall(remain_cols)
        for j in bad_rows[2:end]
            q = xz[j, 2i] / xz[the_row, 2i]
            for (k, l) in enumerate(col_inds)
                xz[j, l] -= q * tmp[k]
            end
        end
...
function contract_sites!(state::StabState{Zp{p}}, I, J) where p
    # check_xz(state.xz)
    cliff = Clifford(Zp{p}[1 0 0 p-1; 0 1 0 0; 0 0 0 1; 0 1 p-1 0])
    for (i, j) in zip(I, J)
...
function reorder_sites(state::StabState{T}, new_order) where T
    n = nsites(state)
    xz = state.xz
    new_xz = zeros(T, 2n, 2n)
...
function ee_on_all_cuts(state::StabState)
    ...
    c_rks = zp_all_vertical_cut_ranks(mat)[2n:-2:2]
